...
namespace fdm 
{
	...
	public:
		...
		inline static std::unique_ptr<Entity> createWithAttributes(msvc_string& entityName, msvc_string& pos, nlohmann::json& attributes) 
		{
			return reinterpret_cast<std::unique_ptr<Entity> (__fastcall*)(msvc_string& entityName, msvc_string& pos, nlohmann::json& attributes)>(getFuncAddr((int)Func::Entity::createWithAttributes))(entityName, pos, attributes);
		}
		nlohmann::json save() 
		{
		...
		inline static std::unique_ptr<Entity> instantiateEntity(const msvc_string& itemName, const uuid& id, glm::vec4& pos, const msvc_string& type, nlohmann::json& attributes) 
		{
			return reinterpret_cast<std::unique_ptr<Entity> (__fastcall*)(const msvc_string& itemName, const uuid& id, glm::vec4& pos, const msvc_string& type, nlohmann::json& attributes)>(getFuncAddr((int)Func::Entity::instantiateEntity))(itemName, id, pos, type, attributes);
		}
		...
		virtual msvc_string getName() { return ""; }
