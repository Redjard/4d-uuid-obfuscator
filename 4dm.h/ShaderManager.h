...
namespace fdm 
{
	...
	{
	public:
		inline static std::map<msvc_string,Shader *>* shaders = reinterpret_cast<std::map<msvc_string,Shader *>*>((base + 0x2C0728));
		...
		inline static const Shader* get(const msvc_string& shaderName)
		{
			return reinterpret_cast<const Shader * (__fastcall*)(const msvc_string&)>(
				getFuncAddr((int)Func::ShaderManager::get)
				)(shaderName);
		}

		inline static bool loadFromShaderList(const msvc_string& jsonListPath) 
		{
			return reinterpret_cast<bool (__fastcall*)(const msvc_string& jsonListPath)>(getFuncAddr((int)Func::ShaderManager::loadFromShaderList))(jsonListPath);
		}
	};
}
