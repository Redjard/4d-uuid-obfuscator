...
namespace fdm 
{
	...
	public:
		...
		inline static std::unique_ptr<Item> instantiateItem(const msvc_string& itemName, uint32_t count, const msvc_string& type, nlohmann::json& attributes) 
		{
			return reinterpret_cast<std::unique_ptr<Item> (__fastcall*)(const msvc_string& itemName, uint32_t count, const msvc_string& type, nlohmann::json& attributes)>(getFuncAddr((int)Func::Item::instantiateItem))(itemName, count, type, attributes);
		}
