...
namespace fdm 
{
	...
	public:
		...
		void addChatMessage(Player* player, const msvc_string& message, uint32_t color) 
		{
			return reinterpret_cast<void (__thiscall*)(StateGame* self, Player* player, const msvc_string& message, uint32_t color)>(getFuncAddr((int)Func::StateGame::addChatMessage))(this, player, message, color);
		}
	};
}
